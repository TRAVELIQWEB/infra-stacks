...
###############################################
# 12) Create NON-INTERACTIVE runner script
###############################################
RUNNER="/opt/mongo-backups/${MONGO_PORT}/run-oplog-backup.sh"

cat > "$RUNNER" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

PORT_DIR="$(dirname "$0")/oplog"
CONFIG_FILE="${PORT_DIR}/oplog-config.env"
STATE_FILE="${PORT_DIR}/state/last_ts.json"
...
###############################################
# 13) Install cron (every minute)
###############################################
CRON_LINE="* * * * * $RUNNER >> /var/log/mongo-oplog-${MONGO_PORT}.log 2>&1"

( crontab -l 2>/dev/null | grep -v "$RUNNER" ; echo "$CRON_LINE" ) | crontab -

sudo touch "/var/log/mongo-oplog-${MONGO_PORT}.log"
sudo chown "$(whoami)":"$(whoami)" "/var/log/mongo-oplog-${MONGO_PORT}.log"

echo -e "${GREEN}Cron installed: every minute oplog backup${RESET}"
